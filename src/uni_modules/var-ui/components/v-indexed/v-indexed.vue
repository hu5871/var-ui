...
<template>
  <view :class="[bem.b(), 'flex', 'flex-col', 'relative', 'flex-grow-1', 'height-0', 'overflow-hidden']">
    <scroll-view :offset-accuracy="5" :scroll-with-animation="false" :scroll-into-view="'id_' + currentId"
      :scroll-top="scrollTop" :class="[bem.e('scroll')]" :scroll-y="true" :style="scrollViewStyle" @scroll="change">
      <slot></slot>
    </scroll-view>
    <view   :id="barId" class="absolute top-50 flex pb-5  right-2 shadow round-15 px-12 overflow-hidden"
      :style="{ transform: 'translateY(-50%)', 'max-height': '80%', 'background-color': props.lineBgColor }">
      <scroll-view :offset-accuracy="5" :scroll-top="barScrollTop"  :scroll-y="true" :scroll-with-animation="true" :show-scrollbar="false">
        <view v-for="item in indexList" :key="item.id" class="flex flex-col" @click.stop="lineNavClick(item)">
          <v-text :color="currentId == item.id ? props.lineActiveColor : 'black'" _class="cursor-pointer"
            :label="item.id"></v-text>
        </view>
      </scroll-view>
    </view>
    <view>
      <v-card v-if="noticeIdShow" :width="100" :height="100"
        class="absolute top-50 flex jusitfy-center overflow-hidden rounded items-center  left-50 -transform-50 shadow-10"
        :shadow="2.4">
        <v-text :color="props.lineActiveColor" :font-size="40" _class="absolute top-50 left-50 -transform-50"
          :label="currentId"></v-text>
      </v-card>
    </view>
  </view>
</template>
...
const lineNavClick = (item: ItemKeys) => {
  ...
  barScrollTop.value = (index - 2) * (barHeight.value || 0) / indexList.value.length;
  const targets = indexList.value.slice(0, index)
  const top = targets.reduce((total, current) => {
